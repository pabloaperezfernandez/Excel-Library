...
Public Function CreateProgramDirectoryTree(RootAppDir As String) As Variant
    If DirectoryExistsQ(RootAppDir) Then
        MkDir FileNameJoin(Array(RootAppDir, "Code"))
        MkDir FileNameJoin(Array(RootAppDir, "Inputs"))
        MkDir FileNameJoin(Array(RootAppDir, "Output"))
        MkDir FileNameJoin(Array(RootAppDir, "Archive"))
        MkDir FileNameJoin(Array(RootAppDir, "Documentation"))
    End If
End Function
...
' RETURNED VALUE
' The requested directory structure is created.
Public Function CreateDirectoryStructure(TheRootDir As String, DirectoryNameTree As Variant) As Variant
    Dim var As Variant
    Dim FullPathDir As String

    Let CreateDirectoryStructure = Null
    
    If Not IsArray(DirectoryNameTree) Then Exit Function
    If Not StringQ(First(DirectoryNameTree)) Then Exit Function
    If Not IsArrayQ(Last(DirectoryNameTree)) Then Exit Function
    
    Let CreateDirectoryStructure = FileNameJoin(Array(TheRootDir, First(DirectoryNameTree)))
