...
Public Sub SelectUsingSqlTestForEmptyRecordSets()
    Dim wbk As Workbook
    Dim data As Variant
    Dim TheQuery As String
    ...
    Let TheQuery = "SELECT * FROM [IdListLo$] WHERE [Cpt_No] = '123';"
    Let data = SelectUsingSql(TheQuery, wbk.FullName)
    Debug.Print TheQuery
    PrintArray data
    Debug.Print
    
    Let TheQuery = "SELECT * FROM [IdListLo$] WHERE [Cpt_No] = '017447';"
    Let data = SelectUsingSql(TheQuery, wbk.FullName)
    Debug.Print TheQuery
    PrintArray data
    Debug.Print
