...
' Returns Null if AStringArray fails both IsArray and String1DArrayQ
Public Function StringJoin(AStringArray As Variant, Optional StringOrStringArray As Variant) As Variant
    Dim NewString As String
    ...
    ' Exit with Null if AStringArray fails IsArray
    If Not IsArray(AStringArray) Then
        Let StringJoin = Null
        Exit Function
    End If
    
    ' Exit with Null if AStringArray fails String1DArrayQ
    If Not String1DArrayQ(AStringArray) Then
        Let StringJoin = Null
        Exit Function
    End If
    
    ' check StringOrStringArray for consistency
    If Not IsMissing(StringOrStringArray) Then
         ' Exit with Null if AStringArray fails IsArray and StringQ
        If Not (IsArray(StringOrStringArray) Or StringQ(StringOrStringArray)) Then
            Let StringJoin = Null
            Exit Function
        End If
        
        ' Exit with Null if AStringArray is an array and fails String1DArrayQ
        ' or its length is different from AStringArray
        If IsArray(StringOrStringArray) Then
            If Not String1DArrayQ(StringOrStringArray) Or _
               GetArrayLength(AStringArray) <> GetArrayLength(StringOrStringArray) Then
    ...
    ' Process the case of StringOrStringArray not passed
    If IsMissing(StringOrStringArray) Then
        ' Initialize the return string to an empty string
        Let NewString = ""
        ...
        Let StringJoin = NewString
        
        Exit Function
    End If
    
    ' Process the cases of StringOrStringArray passed
    
    ' Process case of StringOrStringArray being a string
    If StringQ(StringOrStringArray) Then
        Let AnArray = AStringArray
        
        For i = LBound(AStringArray) To UBound(AStringArray)
            Let AnArray(i) = AnArray(i) & StringOrStringArray
        Next i
    ...
    ' Process case of AStringArray and StringOrStringArray being string arrays
    Let AnArray = AStringArray
        
    If LBound(StringOrStringArray) = 0 And LBound(AStringArray) = 1 Then
        Let IndexOffset = -1
    ElseIf LBound(StringOrStringArray) = 1 And LBound(AStringArray) = 1 Then
        Let IndexOffset = 0
    ElseIf LBound(StringOrStringArray) = 1 And LBound(AStringArray) = 1 Then
        Let IndexOffset = 0
    Else
    ...
    For i = LBound(AStringArray) To UBound(AStringArray)
        Let AnArray(i) = AnArray(i) & StringOrStringArray(IndexOffset + i)
    Next i
