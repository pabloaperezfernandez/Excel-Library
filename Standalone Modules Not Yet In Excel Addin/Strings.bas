...
Public Function StringJoin(StringOrStringArray1 As Variant, Optional StringOrStringArray2 As Variant) As Variant
    ...
    ElseIf LBound(StringOrStringArray1) = 1 And LBound(StringOrStringArray2) = 1 Then
        Let IndexOffset = 0
    ElseIf LBound(StringOrStringArray1) = 0 And LBound(StringOrStringArray2) = 0 Then
        Let IndexOffset = 0
    ElseIf LBound(StringOrStringArray1) = 1 And LBound(StringOrStringArray2) = 0 Then
        Let IndexOffset = -1
    End If
...
' This function takes a 1D range and trims its contents
Public Function TrimArray(TheArray As Variant) As Variant
    Dim i As Long
    Dim j As Long
    Dim ResultsArray As Variant
    ...
    ' Exit with an empty array if TheArray is empty
    If EmptyArrayQ(TheArray) Then
        Let TrimArray = EmptyArray()
        Exit Function
    End If
    
    If NumberOfDimensions(TheArray) = 0 Then
        Let ResultsArray = Trim(TheArray)
    ElseIf NumberOfDimensions(TheArray) = 1 Then
        Let ResultsArray = TheArray
        
        For i = LBound(TheArray) To UBound(TheArray)
            Let ResultsArray(i) = Trim(TheArray(i))
        Next i
    ElseIf NumberOfDimensions(TheArray) = 2 Then
        Let ResultsArray = TheArray
        ...
        For i = LBound(TheArray, 1) To UBound(TheArray, 1)
            For j = LBound(TheArray, 2) To UBound(TheArray, 2)
                Let ResultsArray(i, j) = Trim(TheArray(i, j))
            Next j
        Next i
    Else
        Let ResultsArray = TheArray
...
' DESCRIPTION
' Converts the given 1D array of strings into VBA code. This is a useful function to extract the
' formulas coded by a user in a listobject and then use them programmatically.  This is necessary
' to extract the formulas and control them programmatically to safeguard them from potential
' corruption of the host workbook.
'
' PARAMETERS
' 1. String1DArray - The string to process
'
' RETURNED VALUE
' VBA code to representing the string
Public Function Convert1DArrayOfStringToCode(ByVal String1DArray As Variant) As String
    Dim c As Long
    
    If Not StringArrayQ(String1DArray) Then Exit Function
    
    For c = 1 To Length(String1DArray)
        Let String1DArray = SetPart(String1DArray, _
                                    """" & Replace(Part(String1DArray, c), """", """""") & """", _
                                    c _
                                   )
    Next
    
    Let Convert1DArrayOfStringToCode = "Array(" & Join(String1DArray, ", ") & ")"
End Function
