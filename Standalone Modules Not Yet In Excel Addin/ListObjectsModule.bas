...
Public Sub AppendToListObject(TheData As Variant, lo As ListObject)
    Dim OriginalNumRows As Long
    Dim OriginalNumColumns As Long
    
    Let OriginalNumRows = lo.ListRows.Count
    Let OriginalNumColumns = lo.ListColumns.Count
    Call DumpInSheet(TheData, lo.Range(1, 1).Offset(1 + lo.ListRows.Count, 0))
    Call lo.Resize(lo.Range.Resize(OriginalNumRows + Length(TheData) + 1, OriginalNumColumns))
End Sub
...
Public Function GetVisibleSpecialValues(lo As ListObject) As Variant
    ...
    ' It is easiest to add the header row and the stack on top of each of the ranges specified
    ' by the addresses in TheAddresses.  This will result in the duplicaiton of the header row
    ' because visible cells includes he headers row.  We then get rid of the top row before
    ' returning the result of the function's computation.
    ...
    Let GetVisibleSpecialValues = Rest(ResultArray)
End Function

' DESCRIPTION
' Executes a querry and dumps the resulting data set in the listobject specified.
' The listobject's databody range is cleared before populating it with the new data.
' It returns Null if there is an error.  Returns True on success.
'
' PARAMETERS
' 1. lo - A reference to a listobject
' 2. LambdaOrDataSet - This can be an instance of lambda, the name of a function, or a
'    2D atomic array
' 3. WorksheetPassword (Optional, vbNullString default) - Unprotects and the protects
'    given worksheet with given password if not a null string.
'
' RETURNED VALUE
' Dumped querry results in chosen listobject. Returns Null on error and True on success
Public Function DumpDataInListObject(lo As ListObject, _
                                     LambdaOrDataSet As Variant, _
                                     Optional WorksheetPassword As String = vbNullString) As Variant
    If Not (StringQ(LambdaOrDataSet) Or LambdaQ(LambdaOrDataSet) Or _
            AtomicTableQ(LambdaOrDataSet) _
           ) Then
        Let DumpDataInListObject = Null
    End If
                                
    ' Unprotect worksheet if it is protected
    If WorksheetPassword <> vbNullString Then
        Call UnprotectWorksheet(lo.Parent, WorksheetPassword)
    End If

    ' Turn off listobject's autofilters and Clear listobject's databodyrange
    Let lo.ShowAutoFilter = False
    Call ClearListObjectDataBodyRange(lo)
    
    ' Proceed depending on the type of parameter LambdaOrDataSet
    If StringQ(LambdaOrDataSet) Then
        Call AppendToListObject(Rest(Application.Run(LambdaOrDataSet)), lo:=lo)
    ElseIf LambdaQ(LambdaOrDataSet) Then
        Call AppendToListObject(Rest(Application.Run(LambdaOrDataSet.FunctionName)), lo)
    Else
        Call AppendToListObject(Rest(LambdaOrDataSet), lo)
    End If
                     
    ' Turn off listobject's autofilters and ensure all data shown
    Let lo.ShowAutoFilter = False

    ' Protect the worksheet if a password was given
    If WorksheetPassword <> vbNullString Then
        Call ProtectWorksheet(lo.Parent, WorksheetPassword)
    End If
    
    ' Return True to signal the successful execution of this function.
    Let DumpDataInListObject = True
End Function

Public Sub TurnOnAutoFilters(lo As ListObject)
    ' Turn on autofilters on listobject and show all data
    Let lo.ShowAutoFilter = True
    Call lo.AutoFilter.ShowAllData
End Sub

Public Sub TurnOffAutoFilters(lo As ListObject)
    ' Turn on autofilters on listobject and show all data
    Let lo.ShowAutoFilter = False
End Sub

Public Function AutoFiltersQ(lo As ListObject) As Boolean
    Let AutoFiltersQ = lo.ShowAutoFilter
End Function
