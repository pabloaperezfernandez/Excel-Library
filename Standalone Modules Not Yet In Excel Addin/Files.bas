...
' The first part is a drive (single character drive like c:\, per Windows conventions)
' if it is a single letter and has a colon as the right-most character, and the path
' is Windows' (i.e., "\").  The first part is a server name in ("\\ServerName" in
' Windows networks) if it starts with "\\" and the path is Windows' (i.e., "\"). It is
' a protocol ("https://" in URNs). Protocols only make sense in the Unix convention.
'
' INPUT
...
Public Function FileNameJoin(ArrayOfFileNames As Variant, Optional UnixPathSeparatorQ As Boolean = False, _
                             Optional ParamConsistencyChecksQ As Boolean = False) As String
    Dim c As Long
    Dim ResultArray As Variant
    Dim thePathSeparator As String

    ' Default return value
    Let FileNameJoin = vbNullString

    ' Set the appropriate path separator based on the OS or the user's choice
    Let thePathSeparator = Application.PathSeparator
    If UnixPathSeparatorQ Then Let thePathSeparator = "/"
    ...
    For c = LBound(ArrayOfFileNames) To UBound(ArrayOfFileNames)
        If StrComp(ArrayOfFileNames(c), vbNullString) Then
            Let ResultArray = ConcatenateArrays(ResultArray, _
                                                FilterValueFromArray(Split(ArrayOfFileNames(c), thePathSeparator), _
                                                                     vbNullString))
        End If
    Next
    ...
    ' Handle the case of protocol like http://
    If InStr(First(ResultArray), ":") > 0 And thePathSeparator = "/" Then
        Let ResultArray(LBound(ResultArray)) = ResultArray(LBound(ResultArray)) & "/"
    End If
    
    ' Join the parts and separate them with the file separator string
    Let FileNameJoin = Join(ResultArray, thePathSeparator)
    
    ' This takes care of the case of network server directories in PCs or Windows environments
    If Left(Trim(First(ArrayOfFileNames)), 2) = "\\" Then Let FileNameJoin = "\\" & FileNameJoin
End Function

' DESCRIPTION
' This function splits the given filename into its components. The first part of the
' filename is considered a drive letter (single character drive like c:\, per Windows
' conventions), the server name in ("\\ServerName" in Windows networks) or a protocol
' ("https://" in URNs).  Dive letters only make sense in Windows and must be one character
' long. Protocols only make sense in the Unix convention. In both cases, they are returned
' without any slashes as the first part of filename. Server names like "\\ServerName" are
' returned unchanged as the first part of the filename (e.g., as "\\ServerName").
'
' INPUT
...
Public Function FileNameSplit(aFileName As String, Optional UnixPathSeparatorQ As Boolean = False) As Variant
    Dim thePathSeparator As String
    Dim ResultArray As Variant
    
    ' Set the default return value when aFileName is an empty string
    Let FileNameSplit = EmptyArray()
    
    ' Exit
    If aFileName = vbNullString Then
        Exit Function
    Else
        Let aFileName = Trim(aFileName)
    End If
    ...
    ' Split string along thePathSeparator and trim every filename part
    Let ResultArray = TrimArray(Split(aFileName, thePathSeparator))
    
    ' This handles a filename/path to a network drive in Windows environment
    If Left(aFileName, 2) = "\\" Then
        Let ResultArray = Rest(ResultArray)
        Let ResultArray(LBound(ResultArray)) = "\\" & ResultArray(LBound(ResultArray))
    End If
    
    ' Handle a URL staring with a protocol
    If InStr(First(ResultArray), ":") > 0 And InStr(First(ResultArray), "//") > 0 And thePathSeparator = "/" Then
        Let ResultArray(LBound(ResultArray)) = ResultArray(LBound(ResultArray)) & "//"
    End If

    Let FileNameSplit = ComplementOfSets(ResultArray, Array(vbNullString))
End Function
