...
' 5. IBM 2 1/2 01/27/22 US -> IBM 2 1/2 01/27/22
    Debug.Assert EqualQ(GetTicker("IBM US Equity"), "IBM")
    Debug.Assert EqualQ(GetTicker("US4592001014"), "US4592001014")
    Debug.Assert EqualQ(GetTicker("US4592001014 US Equity"), "US4592001014")
    Debug.Assert EqualQ(GetTicker("IBM 2 1/2 01/27/22 US Equity"), "IBM 2 1/2 01/27/22")
    Debug.Assert EqualQ(GetTicker("IBM 2 1/2 01/27/22 US"), "IBM 2 1/2 01/27/22")
End Sub
...
Public Sub TestPredicatesListColumnQ()
    Dim lc As ListColumn
    Dim lo As ListObject
    
    Call TempComputation.UsedRange.ClearFormats
    Call TempComputation.UsedRange.ClearContents
    Call DumpInSheet([{"col1", "col2"; 1,2;10,20;100,200}], TempComputation.Range("A1"))
    Set lo = AddListObject(TempComputation.Range("A1"), "Table1")
    
    Debug.Assert Not ListColumnQ(lc)
    Debug.Assert Not ListColumnQ(lo)
    Set lc = lo.ListColumns(1)
    Debug.Assert ListColumnQ(lc)
    
    Call TempComputation.UsedRange.ClearFormats
    Call TempComputation.UsedRange.ClearContents
End Sub

Public Sub TestPredicatesListRowQ()
    Dim LR As ListRow
    Dim lo As ListObject
    ...
    Debug.Assert Not ListRowQ(LR)
    Debug.Assert Not ListRowQ(lo)
    Set LR = lo.ListRows(1)
    Debug.Assert ListRowQ(LR)
...
Public Sub TestPredicatesRangeQ()
    Dim var As Variant
    Dim ARange As Range
    Dim InitializedRange As Range
    
    Set InitializedRange = TempComputation.Range("A1")
    
    Debug.Assert ARange Is Nothing
    Debug.Assert Not RangeQ(ARange)
    Debug.Assert RangeQ(InitializedRange)
End Sub
...
Public Sub TestFunctionalProgrammingFilter()
    PrintArray ArrayFilter(NumericalSequence(1, 100), "FilterHelper1")
    PrintArray ArrayFilter(Array("aPAPa", "bbbPAP", "sdgasdkh"), "FilterHelper2")
    PrintArray ArrayFilter(Array("aPAPa", "bbbPAP", "sdgasdkh"), Lambda("x", "", "InStr(1, x, ""PAP"")"))
End Sub
...
' Arrays
'********************************************************************************************
Public Sub TestRemoveDuplicatedSpaces()
    Debug.Assert EqualQ(RemoveDuplicatedSpaces("P   blo  "), "P blo ")
    Debug.Assert EqualQ(RemoveDuplicatedSpaces("     P   blo  "), " P blo ")
End Sub
...
' Arrays
'********************************************************************************************
Public Sub TestFilterValueFromArray()
    Dim AnArray As Variant
    
    Debug.Assert NullQ(FilterValueFromArray(AnArray, vbNullString))
    Debug.Assert NullQ(FilterValueFromArray(Array(), vbNullString))
    Debug.Assert EqualQ(FilterValueFromArray(Array("Pablo", "", "Perez"), vbNullString), Array("Pablo", "Perez"))
    Debug.Assert Not EqualQ(FilterValueFromArray(Array("Pablo", "", "Perez"), vbNullString), Array("Pablo", "", "Perez"))
End Sub
...
'********************************************************************************************
Public Sub TestStringsMakeRoutineName()
    Debug.Assert "'ExcelLibrary9Dot92.xlsb'!MyMod.MyFunc" = _
...
' VbaCodeManipulation
'********************************************************************************************
Public Sub TestClearLambdaFunctionsData()
    Dim l1 As Lambda
    Dim l2 As Lambda
    Dim L3 As Lambda
    ...
    Debug.Print "We created lambdas " & _
                PrintArray(Array(l1.FunctionName, vbCrLf, _
                                 l2.FunctionName, vbCrLf, _
                                 L3.FunctionName, vbCrLf), _
                           True)
                
    Let LambdaName = Last(Split(l1.FunctionName, "."))
    Debug.Print "The lambda's name is -" & LambdaName & "-"
    
    Debug.Print l1.FunctionName & " existence is " & _
                FunctionExistsQ(ThisWorkbook, "LambdaFunctionsTemp", LambdaName)
    
    Call DeleteFunction(ThisWorkbook, "LambdaFunctionsTemp", LambdaName)
    
    Debug.Print vbCrLf & "Deleted function " & l1.FunctionName & vbCrLf

    Debug.Print l1.FunctionName & " existence is " & _
                FunctionExistsQ(ThisWorkbook, "LambdaFunctionsTemp", LambdaName)
    
    Call ClearLambdaFunctionsData
    
    Debug.Print "Deleted the lambdas"
End Sub
...
Public Sub TestTypeConversionsCastShapesToArray()
    ...
    Call TheUI.DeleteShapes(CastShapesToArray(TempComputation.Shapes))
...
Public Sub TestUiDistributeButtonsHorizontally()
    ...
    Call CreateButtonsGrid(TempComputation, TempComputation.Range("B2"), _
                           1, 10, 10, 40, 40, _
                           Array("B1", "B2", "B3"), _
...
Public Sub TestUiCreateValidatedDropDownCell()
    Dim lo As ListObject
    Dim lc As ListColumn
    ...
    Call DumpInSheet([{"col1", "col2", "Col3"; 1,2,3;10,20,30;100,200,300}], TempComputation.Range("A1"))
    Set lo = AddListObject(TempComputation.Range("A1"), "Table1")
    Set lc = lo.ListColumns("Col1")
    
    Call CreateValidatedDropDownCell(TempComputation.Range("B10"), _
                                     "Label1", _
                                     lc)
    Call CreateValidatedDropDownCell(TempComputation.Range("B11"), _
                                     "Label2", _
                                     lc.DataBodyRange.Offset(0, 1))
    Call CreateValidatedDropDownCell(TempComputation.Range("B12"), _
                                     "Label2", _
                                     Flatten(lc.DataBodyRange.Offset(0, 2).Value2))
                                     
    Call TempComputation.UsedRange.ClearFormats
    Call TempComputation.UsedRange.ClearContents
    Call TempComputation.UsedRange.Delete
End Sub
...
Public Sub TestUiSurroundControlsWithGroupBox()
    ...
    Call DeleteShapes(TempComputation.Shapes)
      
    ' Add a button
    Call TempComputation.Buttons.Add(157, 22.5, 22, 9)
    TempComputation.Shapes(1).ScaleWidth 3, msoFalse, msoScaleFromTopLeft
    TempComputation.Shapes(1).ScaleHeight 4.3333333333, msoFalse, msoScaleFromTopLeft
    
    ' Add a second button
    Call TempComputation.Buttons.Add(257, 67.5, 77.5, 47.5)
    
    ' Add a scrollbar
    Call TempComputation.ScrollBars.Add(385, 77.5, 26, 78)
    TempComputation.Shapes(3).ScaleWidth 0.8269230769, msoFalse, msoScaleFromTopLeft
    TempComputation.Shapes(3).ScaleHeight 0.5897435897, msoFalse, msoScaleFromTopLeft

    ' Populate the shape array
    For i = 1 To TempComputation.Shapes.Count
        Set ShapeArray(i) = TempComputation.Shapes(i)
    Next
    
    '
    Call SurroundControlsWithGroupBox(ShapeArray, "The Group Box")
End Sub
...
Public Sub TestGetFileNamesInTree()
    Dim TheFileNames As Variant
    Dim TheDir As String
    
    Let TheDir = "C:\Users\pablo.perez.GLOBAL\OneDrive\WorkProduct\SourceCode\VBA\Excel-Library"
    Let TheFileNames = GetFileNamesInTree(TheDir)
    Debug.Print "There are " & Length(TheFileNames) & " in " & TheDir
    Debug.Print "They are:"
    PrintArray Transpose(Take(GetFileNamesInTree("C:\Users\pablo.perez.GLOBAL\OneDrive\WorkProduct\SourceCode\VBA\Excel-Library"), 86))
End Sub

Public Sub TestDirectorySequence()
    Dim TheSeq As Variant
    Dim TheDir As String
    
    Let TheDir = "C:\Users\pablo.perez.GLOBAL\OneDrive\WorkProduct\SourceCode\VBA\Excel-Library"
    Debug.Assert EqualQ(DirectorySequence(TheDir), _
                        Array("C:", "C:\Users", "C:\Users\pablo.perez.GLOBAL", _
                              "C:\Users\pablo.perez.GLOBAL\OneDrive", _
                              "C:\Users\pablo.perez.GLOBAL\OneDrive\WorkProduct", _
                              "C:\Users\pablo.perez.GLOBAL\OneDrive\WorkProduct\SourceCode", _
                              "C:\Users\pablo.perez.GLOBAL\OneDrive\WorkProduct\SourceCode\VBA", _
                              "C:\Users\pablo.perez.GLOBAL\OneDrive\WorkProduct\SourceCode\VBA\Excel-Library"))
End Sub

Public Sub TestCreateDirectoryTree()
    ' Example 1
    Let tree = Array("Dir1", Array(Array("Dir1-1", Array())))
    Call CreateDirectoryTree("C:\Users\pablo.perez\Desktop", tree)
    
    ' Example 2
    Let tree = Array("Dir1", Array(Array("Dir1-1")))
    Call CreateDirectoryTree("C:\Users\pablo.perez\Desktop", tree)
    
    ' Example 3
    Let tree = Array("Dir10", Array(Array("Dir11", Array()), _
                                    Array("Dir12", Array())))
    Call CreateDirectoryTree("C:\Users\pablo.perez\Desktop", tree)
    
    ' Example 4
    Let tree = Array("Dir10", Array(Array("Dir11"), Array("Dir12")))
    Call CreateDirectoryTree("C:\Users\pablo.perez\Desktop", tree)
    
    Let tree = Array("Dir10", Array(Array("Dir11", Array(Array("Dir111", Array()), _
                                                        Array("Dir112", Array()))), _
                                   Array("Dir12", Array(Array("Dir121", Array()), _
                                                        Array("Dir122", Array()), _
                                                        Array("Dir123", Array())))))
    Call CreateDirectoryTree("C:\Users\pablo.perez\Desktop", tree)
    
    ' Example 5
    Let tree = Array("Dir10", Array(Array("Dir11", Array(Array("Dir111"), _
                                                        Array("Dir112"))), _
                                   Array("Dir12", Array(Array("Dir121"), _
                                                        Array("Dir122"), _
                                                        Array("Dir123")))))
    Call CreateDirectoryTree("C:\Users\pablo.perez\Desktop", tree)
End Sub

Public Sub TestGetListObjectFormulas()
    Dim lo As ListObject
    
    Call TempComputation.UsedRange.ClearFormats
    Call TempComputation.UsedRange.ClearContents
    
    Call DumpInSheet([{"Col1", "Col2", "Col3"; 1,2,3; 10,20,30; 100,200,300}], _
                     TempComputation.Range("A1"))
    Set lo = AddListObject(TempComputation.Range("A1"), "Table1")
    
    Let lo.ListColumns(1).DataBodyRange.Formula = "=Len(""100"")*10*RAND()"
    Let lo.ListColumns(3).DataBodyRange.Formula = "=Len(""300"")*30*RAND()"
    Call SaveListObjectFormulas(lo)
    
    PrintArray GetListObjectHeaderFormulas(lo)
End Sub

Public Sub TestSetListObjectHeaderFormulas()
    Dim lo As ListObject
    ...
    Call SetListObjectHeaderFormulas(lo, _
                                     Array("=len(""100"")*10*RAND(shit)", _
                                           vbNullString, _
                                           "=len(""300"")*30*RAND()" _
                                          ) _
                                    )
...
Public Sub TestConvert1DArrayOfStringToCode()
    Dim lo As ListObject
    ...
    Debug.Print Convert1DArrayOfStringToCode(GetListObjectHeaderFormulas(lo))
End Sub

Public Sub TestArraysDotDrop()
    Dim AnArray As Variant
    
    Let AnArray = EmptyArray()
    Debug.Assert EqualQ(Drop(AnArray, EmptyArray()), EmptyArray())
    
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, Array(1)), Array(2, 3, 4, 5))

    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, 1), Array(2, 3, 4, 5))

    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, 2), Array(3, 4, 5))
    
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, 3), Array(4, 5))
    
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, 4), Array(5))
    
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, 5), EmptyArray())
    
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, 6), Null)
    
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, -6), Null)
    
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, -1), Array(1, 2, 3, 4))

    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, -2), Array(1, 2, 3))
    
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, -4), Array(1))
    ...
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, Array(2)), Array(1, 3, 4, 5))
    
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, Array(3)), Array(1, 2, 4, 5))

    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, Array(4)), Array(1, 2, 3, 5))

    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, Array(5)), Array(1, 2, 3, 4))
    
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, Array(-1)), Array(1, 2, 3, 4))
    
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, Array(-2)), Array(1, 2, 3, 5))
    
    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, Array(-3)), Array(1, 2, 4, 5))

    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, Array(-4)), Array(1, 3, 4, 5))

    Let AnArray = Array(1, 2, 3, 4, 5)
    Debug.Assert EqualQ(Drop(AnArray, Array(-5)), Array(2, 3, 4, 5))
End Sub

Public Sub TestTimeSeriesToDbTableToTimeSeriesTable()
    Dim t As Variant
    Dim m As Variant
    
    Let m = [{"Dates","Tags","Data";1,2,3; 10,20,30; 100,200,300; 1000,2000,3000}]
    Let t = DbTableToTimeSeriesTable(DataTableArray:=m, _
                                     DateHeader:="Dates", _
                                     RowTagsHeader:="Tags", _
                                     DataHeader:="Data")
    Call DumpInSheet(m, TempComputation.Range("A1"))
    Call DumpInSheet(t, TempComputation.Range("E1"))
End Sub


Public Sub TestTimeSeriesToDbTableToTimeSeriesTable2()
    Dim t As Variant
    Dim m() As Variant
    Dim i As Integer
    ...
    ReDim m(1 To 2001, 1 To 3)
    Let m = SetPart(m, Array("Dates", "Tags", "Data"), 1, Array(1, 2, 3))
    For i = 1 To 2000
        Let m = SetPart(m, Array(Rnd(), Rnd(), Rnd()), i + 1, Array(1, 2, 3))
    Next
...
Public Sub TestTimeSeriesTableToTimeSeriesDotDbTable()
    Dim t As Variant
    Dim m As Variant
    
    Let t = Pack2DArray(Array(Array("", "D1", "D2"), _
                              Array("ID1", 1, 2), _
                              Array("ID2", 10, 20), _
                              Array("ID3", 100, 200), _
                              Array("ID4", 1000, 2000)))
                        
    Let m = TimeSeriesTableToDbTable(TimeSeriesTable:=t, _
                                     DateHeader:="Dates", _
                                     RowTagsHeader:="Tags", _
                                     DataHeader:="Data")
    Call DumpInSheet(m, TempComputation.Range("A1"))
...
Public Sub TestListObjectsModuleToListObject()
    Dim lo As ListObject
    Dim var As Variant
    
    Call DumpInSheet([{1,2,3;10,20,30;100,200,300}], TempComputation.Range("A1"))
    Call DumpInSheet([{1,2,3;10,20,30;100,200,300}], TempComputation.Range("E1"))
    Call AddListObject(TempComputation.Range("A1"), "Table1")
    Call AddListObject(TempComputation.Range("E1"), "Table2")
    
    For Each var In Array(TempComputation.ListObjects("Table1"), TempComputation.ListObjects("Table2"))
        Call ClearListObjectDataBodyRange(ToListObject(var))
    Next
    
    For Each var In Array(TempComputation.ListObjects("Table1"), TempComputation.ListObjects("Table2"))
        Call ToListObject(var).Delete
    Next
End Sub

Public Sub TestStatisticsMovingAverage()
    Debug.Assert EqualQ(MovingAverage(Range(1, 10), 2), _
                        Array(1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5) _
                       )
                        
    Debug.Assert EqualQ(MovingAverage(Range(1, 10), 3), _
                        Range(2, 9) _
                       )
                       
    Debug.Assert EqualQ(MovingAverage(Range(1, 10), 5), _
                        Range(3, 8) _
                       )
End Sub

Public Sub TestMovingAverage()
    Debug.Assert EqualQ(MovingAverage(Range(1, 10), 2), _
                        Array(1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5) _
                       )
...
Public Sub TestStatisticsSimpleAveragePerformance()
    Dim m As Variant
    
    Let m = Pack2DArray(Array(Array(#1/1/2018#, 1), _
                              Array(#2/1/2018#, 2), _
                              Array(#3/1/2018#, 3), _
                              Array(#4/1/2018#, 4), _
                              Array(#5/1/2018#, 5), _
...
Public Sub TestArraysAdd()
    Debug.Assert EqualQ(Add([{1,2,3;10,20,30;100,200,300}], _
                            [{1000,2000,3000;1000,2000,3000; 1000,2000,3000}] _
                           ), _
                        [{1001, 2002, 3003; 1010, 2020, 3030; 1100, 2200, 3300}] _
                       )
    ...
    Debug.Assert EqualQ(Add([{1000,2000,3000}], _
                            [{1,2,3;10,20,30;100,200,300}] _
                           ), _
                        [{1001, 2002, 3003; 1010, 2020, 3030; 1100, 2200, 3300}] _
                       )
    ...
    Debug.Assert EqualQ(Add([{1,2,3;10,20,30;100,200,300}], _
                        ...
                        [{1001, 1002, 1003; 2010, 2020, 2030; 3100, 3200, 3300}] _
                       )
    ...
    Debug.Assert EqualQ(Add([{1,2,3}], _
                            [{1000,2000,3000}] _
                           ), _
                        [{1001, 2002, 3003}] _
                       )
    ...
    Debug.Assert EqualQ(Add([{1,2,3;10,20,30;100,200,300}], _
                            Array(1000, 2000, 3000) _
                           ), _
                        [{1001, 2002, 3003; 1010, 2020, 3030; 1100, 2200, 3300}] _
                       )
                       
    Debug.Assert EqualQ(Add(Array(1000, 2000, 3000), _
                            [{1,2,3;10,20,30;100,200,300}] _
                           ), _
                        [{1001, 2002, 3003; 1010, 2020, 3030; 1100, 2200, 3300}] _
                       )
                       
    Debug.Assert EqualQ(Add(Array(1, 2, 3), _
                            Array(1000, 2000, 3000) _
                           ), _
                        Array(1001, 2002, 3003) _
                       )
                       
    Debug.Assert EqualQ(Add([{1,2,3;10,20,30;100,200,300}], _
                            1 _
                           ), _
                        [{2,3,4; 11,21,31; 101,201,301}] _
                       )
                       
    Debug.Assert EqualQ(Add(1, _
                            [{1,2,3;10,20,30;100,200,300}] _
                           ), _
                        [{2,3,4; 11,21,31; 101,201,301}] _
                       )
    ...
    Debug.Assert EqualQ(Add(1, 1), 2)
End Sub

Public Sub TestArraysSubtract()
    Debug.Assert EqualQ(Subtract([{1,2,3;10,20,30;100,200,300}], _
                            [{1000,2000,3000;1000,2000,3000; 1000,2000,3000}] _
                           ), _
                        [{-999, -1998, -2997; -990, -1980, -2970; -900, -1800, -2700}] _
                       )
    ...
    Debug.Assert EqualQ(Subtract([{1000,2000,3000}], _
                            [{1,2,3;10,20,30;100,200,300}] _
                           ), _
                        [{999, 1998, 2997; 990, 1980, 2970; 900, 1800, 2700}] _
                       )
    ...
    Debug.Assert EqualQ(Subtract([{1,2,3;10,20,30;100,200,300}], _
                        ...
                        [{-999, -998, -997; -1990, -1980, -1970; -2900, -2800, -2700}] _
                       )
    ...
    Debug.Assert EqualQ(Subtract([{1,2,3}], _
                            [{1000,2000,3000}] _
                           ), _
                        [{-999, -1998, -2997}] _
                       )
    ...
    Debug.Assert EqualQ(Subtract(Array(1000, 2000, 3000), _
                            [{1,2,3;10,20,30;100,200,300}] _
                           ), _
                        [{999, 1998, 2997; 990, 1980, 2970; 900, 1800, 2700}] _
                       )
                       
    Debug.Assert EqualQ(Subtract(Array(1, 2, 3), _
                            Array(1000, 2000, 3000) _
                           ), _
                        Array(-999, -1998, -2997) _
                       )
                       
    Debug.Assert EqualQ(Subtract([{1,2,3;10,20,30;100,200,300}], _
                            1 _
                           ), _
                        [{0,1,2; 9,19,29; 99,199,299}] _
                       )
                       
    Debug.Assert EqualQ(Subtract(1, _
                            [{1,2,3;10,20,30;100,200,300}] _
                           ), _
                        [{0,-1,-2; -9,-19,-29; -99,-199,-299}] _
                       )
    ...
    Debug.Assert EqualQ(Subtract(1, 1), 0)
End Sub

Public Function SampleFunctionReturningTable() As Variant
    Let SampleFunctionReturningTable = _
        [{"col1","col2"; 1,2;10,20; 100, 200; 1000, 2000; 10000, 20000}]
End Function

Public Sub TestDumpDataInListObject()
    Dim lo As ListObject
    Dim t As Variant
    ...
    Let t = [{"col1","col2"; 1,2;10,20}]
    Call DumpInSheet(t, TempComputation.Range("A1"))
    Set lo = AddListObject(TempComputation.Range("A1"))
    
    ' First test, populating with a matrix
    Let t = [{"col1","col2"; 1,2;10,20; 100, 200; 1000, 2000}]
    Call DumpDataInListObject(lo, t)
    
    ' Second test, populating with a function name
    Call DumpDataInListObject(lo, SampleFunctionReturningTable)
    
    ' Third test, using a lambda
    Call DumpDataInListObject(lo, Lambda(Array(), "", "[{""col1"",""col2"";1,2}]"))
End Sub
