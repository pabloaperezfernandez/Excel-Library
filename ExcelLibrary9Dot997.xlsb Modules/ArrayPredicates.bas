...
' DESCRIPTION
' Boolean function returning True if its argument is a dimensioned array all of whose
...
Public Function ElementwiseArithmeticParameterConsistentQ(Arg1 As Variant, Arg2 As Variant)
    ...
    Let nd1 = NumberOfDimensions(Arg1)
    Let nd2 = NumberOfDimensions(Arg2)
    
    Let nr1 = NumberOfRows(Arg1)
    Let nr2 = NumberOfRows(Arg2)
    Let nc1 = NumberOfColumns(Arg1)
    Let nc2 = NumberOfColumns(Arg2)
    Let l1 = Length(Arg1)
    Let l2 = Length(Arg2)
    
    If nd1 > 2 Or nd2 > 2 Then Exit Function
        
    ' Check compatible dimensions for all possible cases.
    If EmptyArrayQ(Arg1) And Not EmptyArrayQ(Arg2) Then Exit Function
    
    If Not EmptyArrayQ(Arg1) And EmptyArrayQ(Arg2) Then Exit Function
    
    If nd1 = 2 And nd2 = 2 Then
        If nr1 = 1 Or nr2 = 1 Then
            If nc1 <> nc2 Then Exit Function
        ElseIf nc1 = 1 Or nc2 = 1 Then
            If nr1 <> nr2 Then Exit Function
        Else
            If nr1 <> nr2 Or nc1 <> nc2 Then Exit Function
        End If
    ElseIf nd1 = 2 And nd2 = 1 Then
        If nc1 <> l2 Then Exit Function
    ElseIf nd1 = 1 And nd2 = 2 Then
        If l1 <> nc2 Then Exit Function
...
' DESCRIPTION
' Boolean function returning True when its parameter is an array that is dimensioned
' and non-empty
'
' PARAMETERS
' 1. TheParam - any Excel object
'
' RETURNED VALUE
' Ture or False depending on whether its argument is a dimensioned, non-empty array
Public Function DimensionedNonEmptyArrayQ(TheParam As Variant) As Boolean
    Let DimensionedNonEmptyArrayQ = False

    ' Error Check: Exit with Null if AnArray is not an array
    If Not IsArray(TheParam) Then Exit Function
    
    ' Error Check: Exit with Null if AnArray is not dimensioned
    If Not DimensionedQ(TheParam) Then Exit Function
    
    ' Error Check: Exit with Null if AnArray is Empty
    If EmptyArrayQ(TheParam) Then Exit Function
    
    Let DimensionedNonEmptyArrayQ = True
End Function
