...
Public Function DumpDataInListObject(lo As ListObject, _
    ...
    ' Turn off listobject's autofilters
    Let lo.ShowAutoFilter = False
    ' Clear listobject's databodyrange
    ...
    If StringQ(LambdaOrDataSet) Then
        Call DumpInSheet(Rest(Application.Run(LambdaOrDataSet)), _
                         lo.Range(1, 1).Offset(1, 0))
    ElseIf LambdaQ(LambdaOrDataSet) Then
        Call DumpInSheet(Rest(Application.Run(LambdaOrDataSet.FunctionName)), _
                         lo.Range(1, 1).Offset(1, 0))
    Else
        Call DumpInSheet(Rest(LambdaOrDataSet), lo.Range(1, 1).Offset(1, 0))
    End If
