...
Public Function GetHistorialBloombergData(securities() As String, _
    ...
    For FieldCounter = 1 To Length(fields)
        ...
        Let ArrayOfTimeSeries(FieldCounter) = TimeSeriesMatrix
    Next

    If Length(fields) = 1 Then
        Let GetHistorialBloombergData = First(ArrayOfTimeSeries)
    Else
        Let GetHistorialBloombergData = ArrayOfTimeSeries
    End If
End Function
