...
Public Function GetXmlElement(TheString As String, TheTag As String, _
    ...
    While CurrentPos < Len(Trim(TheString)) And _
          ((OpeningTagCount = 0 And ClosingTagCount = 0) Or _
        ...
        If (OpeningTagCount <> ClosingTagCount And NextClosingPos = 0) Or _
           (OpeningTagCount + ClosingTagCount = 0 And _
           NextClosingPos = 0) Then
            Let GetXmlElement = ""
            Exit Function
    ...
    If OpeningTagCount = ClosingTagCount And _
       OpeningTagCount + ClosingTagCount <> 0 And _
       IncludeTags = True Then
        Let GetXmlElement = Mid(Trim(TheString), _
                                StartPos, _
                                CurrentPos - StartPos + Len("</" & FixedTag & ">") - 1)
    ElseIf OpeningTagCount = ClosingTagCount And _
           OpeningTagCount + ClosingTagCount <> 0 And _
           IncludeTags = False Then
        Let EndingPos = InStr(StartPos, Trim(TheString), ">")
    
        Let GetXmlElement = Mid(Trim(TheString), _
                                EndingPos + 1, _
                                CurrentPos - EndingPos - 2)
    Else
        Let GetXmlElement = ""
    End If
End Function
...
Public Function GetFirstXmlBracketExpression(TheString As String) As String
    ...
    Let GetFirstXmlBracketExpression = "<" & _
                                       Trim(Mid(TheString, _
                                                StartPos + 1, _
                                                EndPos - StartPos - 1)) & _
                                       ">"
End Function
